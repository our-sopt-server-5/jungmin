...
router.post('/signin', async (req, res) => {
  ...
  if (user[0] === undefined) {
    return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.OK, resMessage.NO_USER));
  }
  ...
  res.status(statusCode.OK).send(util.success(statusCode.OK, resMessage.LOGIN_SUCCESS, { successToken: token }));
});
